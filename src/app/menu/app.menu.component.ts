...
export class AppMenuComponent implements OnInit, AfterViewInit {
    ...
    async ngOnInit() {
        this.model = await [
            ...
            { label: 'Cadastro', icon: 'fa fa-fw fa-sitemap', routerLink: ['/signup'] },
            // {label: 'Horas Extras', icon: 'fa fa-fw fa-clock-o', url: 'assets/pages/access.html', target: '_blank'},
            // {label: 'Configurações', icon: 'fa fa-fw fa-gear', url: 'assets/pages/access.html', target: '_blank'},
            // {
            //     label: 'Menu', icon: 'fa fa-fw fa-bars', badge: 5, badgeStyleClass: 'green-badge',
            //     items: [
            ...
            //             items: [
            //                 {label: 'Cosmic', icon: 'fa fa-fw fa-paint-brush', command: (event) => {this.changeLayout('cosmic'); }},
            //                 {label: 'Lawrencium', icon: 'fa fa-fw fa-paint-brush',
            // command: (event) => {this.changeLayout('lawrencium'); }},
            ...
            //                 {label: 'Flow', icon: 'fa fa-fw fa-paint-brush', command: (event) => {this.changeLayout('flow'); }},
        ...
        // this.loadSystemAccessContributor();
        // this.loadSystemAccessVacancy();
        // this.loadSystemAccessEvaluation();
    }

    async loadSystemAccessContributor() {
        const systems = await JSON.parse(localStorage.getItem('systems'));
        if (systems) {
            await systems.forEach(moduleAccess => {
                if (moduleAccess.name === 'COLABORADORES') {
                    if (BasicUtils.isEmptyList(moduleAccess.functions)) {
                        this.model = this.model.filter((menu) => menu.label !== 'Colaboradores');
                    }
                }
            });
        }
    ...
    async loadSystemAccessVacancy() {
        const systems = await JSON.parse(localStorage.getItem('systems'));
        await systems.forEach(moduleAccess => {
            if (moduleAccess.name === 'VAGAS') {
                if (BasicUtils.isEmptyList(moduleAccess.functions)) {
                    this.model = this.model.filter((menu) => menu.label !== 'Vagas');
                }
            }
        });
    }

    async loadSystemAccessEvaluation() {
        const systems = await JSON.parse(localStorage.getItem('systems'));
        await systems.forEach(moduleAccess => {
            if (moduleAccess.name === 'AVALIACAOES') {
                if (BasicUtils.isEmptyList(moduleAccess.functions)) {
                    this.model = this.model.filter((menu) => menu.label !== 'Gestão de Desempenho');
                }
            }
        });
    }
